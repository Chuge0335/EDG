...
from lvdm.models.samplers.ddim_multimodel import DDIMSampler
from utils.utils import instantiate_from_config
import numpy as np
if os.getenv('use_temp2', '0')!= '0':
    import lvdm
    lvdm.modules.attention.TemporalTransformer = lvdm.modules.attention.TemporalTransformer2
    lvdm.modules.networks.openaimodel3d.TemporalTransformer = lvdm.modules.attention.TemporalTransformer2
...
def image_guided_synthesis_analytic(dynamic_model, consistency_model, T, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    ddim_sampler = DDIMSampler(dynamic_model, consistency_model, T=T)
    ...
    for name, model in zip(
        ['dyn', 'con'],
        [dynamic_model, consistency_model]
    ):
    ...
    for _ in range(n_samples):
        ...
        if ddim_sampler is not None:
            ...
            samples, x_0_preds = ddim_sampler.sample(S=ddim_steps,
                                            ...
                                            unconditional_conditioning=ucs,
                                            eta=ddim_eta,
                                            cfg_img=cfg_img, 
                                            mask=cond_mask,
                                            x0=cond_z0,
...
def load_model(args, gpu_no, ckpt_path):
...
def run_inference(args, gpu_num, gpu_no):
    ...
    dynamic_model = load_model(args, gpu_no, ckpt_path=args.dynamic_path)
    dynamic_model.cond_stage_model.device = dynamic_model.device 
    consistency_model = load_model(args, gpu_no, ckpt_path=args.consistency_path)
    consistency_model.cond_stage_model.device = consistency_model.device 
    model = dynamic_model
    ...
    base_samples_per_gpu = num_samples // gpu_num
    remainder = num_samples % gpu_num

    start_index = base_samples_per_gpu * gpu_no + min(remainder, gpu_no)
    samples_split = base_samples_per_gpu + (remainder > gpu_no)
    end_index = start_index + samples_split
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch', total=len(prompt_list_rank)//args.bs):
            ...
            batch_samples = image_guided_synthesis_analytic(dynamic_model, consistency_model, args.T, prompts, videos, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
            ...
            for nn, samples in enumerate(batch_samples):
                ...
                save_results_seperate(prompt, samples, prompt, fakedir, fps=8, loop=args.loop,M=args.M)
...
def get_parser():
    ...
    parser.add_argument("--dynamic_path", type=str, default=None, help="checkpoint path")
    parser.add_argument("--consistency_path", type=str, default=None, help="checkpoint path")
    parser.add_argument("--T", type=int, default=None, help="checkpoint path")
