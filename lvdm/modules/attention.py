...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
        ...
        if not self.only_self_att:
            self.cc_projection = nn.Linear(1024+inner_dim, inner_dim)
    ...
    def forward(self, x, context=None):
        ...
        if not self.only_self_att:
            context = repeat(context, 'b l con -> (b r) l con', r=(h * w)).contiguous()
            x = self.cc_projection(torch.cat([x, context], dim=-1))
