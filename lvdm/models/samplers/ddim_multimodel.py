...
class DDIMSampler(object):
    def __init__(self, dynamic_model, consistent_model, T,  schedule="linear",**kwargs):
        super().__init__()
        self.consistent_model = consistent_model
        self.model = self.dynamic_model = dynamic_model
        self.T = T
        self.ddpm_num_timesteps = consistent_model.num_timesteps
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if t > self.T:
            c = c['dyn']
            self.model = self.dynamic_model
            unconditional_conditioning = unconditional_conditioning['dyn']
        else:
            c = c['con']
            self.model = self.consistent_model
            unconditional_conditioning = unconditional_conditioning['con']
    ...
    @torch.no_grad()
    def p_sample_ddim_multi(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,
                      uc_type=None, conditional_guidance_scale_temporal=None,mask=None,x0=None,guidance_rescale=0.0,**kwargs):
            ...
            if isinstance(c, torch.Tensor) or isinstance(c, dict):
                for name in ['dyn', 'con']:
                    if name == 'dyn':
                        ...
                        model = self.consistent_model
                    e_t_cond[name] = model.apply_model(x, t, c[name], **kwargs)
                    e_t_uncond[name] = model.apply_model(x, t, unconditional_conditioning[name], **kwargs)
            else:
                raise NotImplementedError
            # w1 = unconditional_guidance_scale * math.cos(math.pi / 2 * t / 1000) ** 2
            # w2 = unconditional_guidance_scale * math.sin(math.pi / 2 * t / 1000) ** 2
            w1 = unconditional_guidance_scale * ( t/1000)
            w2 = unconditional_guidance_scale - w1
            e_t_uncond_ =  (w1 * e_t_uncond['dyn'] + w2 * e_t_uncond['con']) / unconditional_guidance_scale
            e_t_cond_ =  (w1 * e_t_cond['dyn'] + w2 * e_t_cond['con']) / unconditional_guidance_scale
            # e_t_uncond_ = e_t_uncond['dyn']
            # e_t_cond_ = e_t_cond['dyn']
            model_output =  e_t_uncond_ + w1 * (e_t_cond['dyn'] - e_t_uncond['dyn']) + w2 * (e_t_cond['con'] - e_t_uncond['con'])
            
            if guidance_rescale > 0.0:
                model_output = rescale_noise_cfg(model_output, e_t_cond_, guidance_rescale=guidance_rescale)
