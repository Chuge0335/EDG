...
class LatentVisualDiffusion(LatentDiffusion):
    def __init__(self, img_cond_stage_config, image_proj_stage_config, freeze_embedder=True, image_proj_model_trainable=True, *args, **kwargs):
        ...
        if not self.unet_config.params.temporal_selfatt_only :
            self.queries = nn.Parameter(torch.randn(16, 1024))
            self.qformer = CrossAttention(query_dim=1024, heads=8, dim_head=128)
    ...
    def get_batch_input(self, batch, random_uncond,t,return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, **kwargs):
        ...
        if not self.unet_config.params.temporal_selfatt_only :
            ...
            cond["temporal_context"] = [self.qformer(queries_expanded, prompt_imb)]
        out = [z, cond]
        if return_first_stage_outputs:
            xrec = self.decode_first_stage(z)
            out.extend([xrec])
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
        ...
        if sample:
            ...
            if unconditional_guidance_scale != 1.0:
                ...
                ## hybrid case
                if not self.unet_config.params.temporal_selfatt_only :
                    queries_expanded = self.queries.unsqueeze(0).expand(N, -1, -1)
                    temporal_context = self.qformer(queries_expanded, uc_prompt)
                if isinstance(c, dict):
                    uc_hybrid = {"c_concat": [c_cat], "c_crossattn": [uc], "temporal_context": [temporal_context]}
                    uc = uc_hybrid
            else:
                uc = None
